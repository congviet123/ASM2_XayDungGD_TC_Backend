      ...
      <section class="row">
        ...
        <aside class="col-md-4 mb-4" id="user-section">
          <div class="sidebar-card p-4 mb-4 bg-light rounded shadow-sm">
            ...
            <img :src="userAvatar" class="img-fluid rounded-circle mb-3" alt="Ảnh đại diện" style="width:100px;" />
            <h5 class="mb-2">{{ userName }}</h5>
            <p class="mb-2">{{ userEmail }}</p>
            <p class="fst-italic">{{ userQuote }}</p>
          </div>
...
const userAvatar = ref(new URL('@/assets/images/avatar.png', import.meta.url).href)
const userName = ref('TS. Trần Hoàng')
const userEmail = ref('Tổ chức Tư vấn sức khỏe quốc tế độc lập')
const userQuote = ref('"Ta không được chọn nơi mình sinh ra, nhưng ta được chọn cách mình sẽ sống"')
...
function updateUserInfo() {
  ...
  if (user) {
    ...
    userAvatar.value = new URL('@/assets/images/avatar.png', import.meta.url).href
    userQuote.value = user.role === 'Admin' ? 'Quản trị viên hệ thống' : 'Thành viên hệ thống'
  } else {
    userAvatar.value = new URL('@/assets/images/avatar.png', import.meta.url).href
    userName.value = 'TS. Trần Hoàng'
    userEmail.value = 'Tổ chức Tư vấn sức khỏe quốc tế độc lập'
    userQuote.value = '"Ta không được chọn nơi mình sinh ra, nhưng ta được chọn cách mình sẽ sống"'
  }
}
